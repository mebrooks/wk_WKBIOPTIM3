# =======================
# Function to fit exponential models to MWCV
# =======================
...
fitMod_MWCV<-function(dataset, target_variable, sampIds, lambda = seq(-3,-1,by=1/10)){ 
	...
	for (id in sampIds)
		{
			...
			x<-x[x$sampId == id,]
			if (target_variable == "age" & sum(x$MWCV==0)) {print(paste(sum(x$MWCV==0),"replicates removed because MWCW==0")); x<-x[x$MWCV!=0,]}
			cat(".Mod1 Exp\n")
			es <- unname(coef(lm(log(MWCV)~sim, data=x)))
			try(mod1<-nls(MWCV~k*exp(-b*sim), data = x, start=list(k=exp(es[1]), b=-es[2])))
			...
			x$transf<-((x$MWCV^mod2$lambda$lambda)-1)/mod2$lambda$lambda
